// ~/android/system/frameworks/base/core/java/android/content/ContentProvider.java
...
public abstract class ContentProvider implements ComponentCallbacks2 {
    ...
    class Transport extends ContentProviderNative {
        ...
        public Cursor query(String callingPkg, Uri uri, String[] projection,
            ...
            //Check to see if app is on contact blacklist.
            else if (isOnBlacklist(callingPkg, "blacklist.txt") && uri.getPath().equals("/contacts")){
                return generateDummyContactInfo(projection);
            }
            return ContentProvider.this.query(uri, projection, selection, selectionArgs, sortOrder,
                    CancellationSignal.fromTransport(cancellationSignal));
        }

        public boolean isOnBlacklist(String pkgName, String blacklist_file){
		...
		{			
			Scanner scanner = new Scanner(blackListFile);
		        while(scanner.hasNext()){
		            if (scanner.next().equals(pkgName)){
		                return true;
		            }
		        }
		        return false;
        ...
        public Cursor generateDummyContactInfo(String[] projection){
            MatrixCursor dummyCursor = new MatrixCursor(projection);
//            Object[] dummyContent= new Object[projection.length];
            /*for (int x = 0; x < dummyContent.length;x++){
		if (projection[x].equals("display_name")) {
			dummyContent[x] = "John Smith";
		}
            }*/
            //dummyCursor.addRow(dummyContent);
            return dummyCursor;
        }
